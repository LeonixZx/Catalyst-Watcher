...
class CustomTitleBar(QWidget):
    def __init__(self, parent=None):
        ...
        # Title
        self.title_label = QLabel("Advanced Chart")
...
class TradingChartWidget(QWidget):
    ...
    def initUI(self):
        ...
        self.setWindowTitle("Advanced Chart")
        ...
        self.setFixedSize(1000, 600)  # Set a fixed size for the widget
        self.center_top()
    ...
    def position_widget(self, main_window):
        if isinstance(main_window, QWidget):
            ...
            self.move(main_geo.x() + main_geo.width() + 10, main_geo.y())  # Position to the right of the main window
        else:
            # If main_window is not a QWidget, position the widget at the top-right of the screen
            screen = QDesktopWidget().screenNumber(QDesktopWidget().cursor().pos())
            screen_geometry = QDesktopWidget().screenGeometry(screen)
            self.move(screen_geometry.right() - self.width(), screen_geometry.y())

def run_trading_chart_widget():
    ...
    widget = TradingChartWidget()
    widget.show()
    return widget
    
if __name__ == '__main__':
    app = QApplication(sys.argv)
    widget = run_trading_chart_widget()
    sys.exit(app.exec_())