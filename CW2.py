...
class NewsApp:
    ...
    def show_ai_summary(self):
        if not self.filtered_data:
            messagebox.showwarning("No Data", "Please fetch news data before running AI analysis.")
            return
        create_ai_summary_widget(self.master, self.filtered_data)



    def handle_ai_operation(self, choice):
    ...
    def run_ai_analysis(self):
        exe_path = os.path.join(os.getcwd(), "CatalystWatcherAI.exe")
        if os.path.exists(exe_path):
            subprocess.Popen(exe_path)
        else:
            CTkMessagebox(title="Error", message="CatalystWatcher AI.exe not found in the current directory.", icon="cancel")
...
#        ctk.CTkButton(button_frame, text="AI Summary", command=self.show_ai_summary, **orange_button_style).grid(row=1, column=4, padx=2, pady=5)
        ...
        # Copyright label
        copyright_label = ctk.CTkLabel(self.master, text="© Created by LeonCybr Lab | 2024")
